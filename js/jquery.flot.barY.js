 ...
 * Reditributed by Yuxing Li under MIT license, August, 2009.
 */
...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        function drawBarY(x, y, barTop, barBottom, offset, fill, axisx, axisy, c) {
            var drawLeft = false, drawRight = false,
                drawTop = false, drawBottom = false,
                left = 0, right = x,
                bottom = y + barBottom, top = y + barTop;
...
//            }
            ...
            if (fill) {
                c.beginPath();
                c.moveTo(axisx.p2c(left), axisy.p2c(top) + offset);
                c.lineTo(axisx.p2c(right), axisy.p2c(top) + offset);
                c.lineTo(axisx.p2c(right), axisy.p2c(bottom) + offset);
                c.lineTo(axisx.p2c(left), axisy.p2c(bottom) + offset);
                c.fill();
            }
        ...
        function drawSeriesBars(series) {
            function plotBars(data, barLeft, barRight, offset, fill, axisx, axisy) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i] == null)
                        continue;
                    drawBarY(data[i][0], data[i][1], barLeft, barRight, offset, fill, axisx, axisy, ctx);
                }
            }
